...
# -*- coding: utf-8 -*-
"""
Test Script: Teste LIMPO - Simula um fluxo completo de pedido com banco vazio
Teste End-to-End do sistema sem GUI (VERSÃO CLEAN)
"""
...
def limpar_banco():
    """Deleta o banco de dados para começar fresco"""
    db_path = pedidos.DB_PATH
    if db_path.exists():
        db_path.unlink()
    ...
    print("✓ Banco de dados novo criado\n")
...
def teste_listar_pedidos():
    ...
    try:
        ...
        if pedidos_list:
            print(f"✓ Pedidos no banco ({len(pedidos_list)} encontrados):")
...
def teste_database_schema():
    ...
    try:
        ...
        colunas_encontradas = [row[1] for row in colunas]
        ...
        # Verificar se todas as esperadas estão presentes
        faltando = set(colunas_esperadas) - set(colunas_encontradas)
        if faltando:
            print(f"\n✗ Colunas faltando: {faltando}")
            return False
        else:
...
def main():
    ...
    print("\n")
    linha("=", 70)
    print("TESTE COMPLETO LIMPO: SISTEMA DE MARMITAS - KELVIN")
    print("Simulação End-to-End com Banco de Dados Novo")
    linha("=", 70)
